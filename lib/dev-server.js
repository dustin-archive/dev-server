...
const { RELOAD_ADDRESS = 'localhost', RELOAD_PORT = 3000 } = process.env
const mimes = {}
...
function startServer (http, ws) {
  ...
  http.listen(RELOAD_PORT, RELOAD_ADDRESS, () => {
    const client = new WebSocket(`ws://${RELOAD_ADDRESS}:${RELOAD_PORT}`)
    client.on('error', err => echo(err.message, console.error))
    client.on('open', () => {
      echo(`Waiting for reloads at http://${RELOAD_ADDRESS}:${RELOAD_PORT}`)
      startWatch(client)
    })
  })
}
    ...
    watch(glob.slice(0, baseEnd), file => {
      if (minimatch(file, glob)) {
        if (command) {
          ...
          sub.on('error', err => {
            client.send(JSON.stringify({ name: 'error', message: err.message }))
          })
          ...
          sub.on('exit', code => {
            if (code === 0 && !stderr) {
              client.send(JSON.stringify({ name: 'update', message: file }))
            } else if (!silent) {
              client.send(JSON.stringify({ name: 'error', message: stderr }))
            }
          })
        } else {
          client.send(JSON.stringify({ name: 'update', message: file }))
...
startServer(httpServer, wsServer)
